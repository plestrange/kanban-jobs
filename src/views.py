...
def days_in_stage(card: Card, today: date) -> int:
    """`history[-1].occurred` is the clock — see docs/ARCHITECTURE.md §5.2 / §6.1."""
    if not card.history:
...
def board(cards: list[Card], today: date | None = None) -> dict[str, list[Card]]:
    """Columns, oldest first within each — the only sort (docs/ARCHITECTURE.md §5.6)."""
    today = today or date.today()
    columns: dict[str, list[Card]] = {stage: [] for stage in STAGES}
    for card in cards:
        assert card.stage is not None, f"{card.id} has no stage — inbox cards don't belong here"
